...
public class DefaultXmlBuilder implements IVisitor {
   ...
   public void visitDatabaseReport(DatabaseReport databaseReport) {
      startTag(ENTITY_DATABASE_REPORT, null, ATTR_DATABASE, databaseReport.getDatabase(), ATTR_CONNECT_URL, databaseReport.getConnectUrl(), ATTR_STARTTIME, toString(databaseReport.getStartTime(), "yyyy-MM-dd HH:mm:ss"), ATTR_ENDTIME, toString(databaseReport.getEndTime(), "yyyy-MM-dd HH:mm:ss"));

      if (!databaseReport.getDatabaseNames().isEmpty()) {
         for (String databaseName : databaseReport.getDatabaseNames().toArray(new String[0])) {
            tagWithText(ELEMENT_DATABASENAME, databaseName);
         }
      }

      if (!databaseReport.getDomainNames().isEmpty()) {
         for (String domainName : databaseReport.getDomainNames().toArray(new String[0])) {
            tagWithText(ELEMENT_DOMAINNAME, domainName);
         }
      }

      if (!databaseReport.getDomains().isEmpty()) {
         for (Domain domain : databaseReport.getDomains().values().toArray(new Domain[0])) {
            visitDomain(domain);
         }
      }

      endTag(ENTITY_DATABASE_REPORT);
   }
   ...
   public void visitDomain(Domain domain) {
      ...
      if (!domain.getTables().isEmpty()) {
         for (Table table : domain.getTables().values().toArray(new Table[0])) {
            visitTable(table);
         }
      }
