...
public class DefaultXmlBuilder implements IVisitor {
   ...
   public void visitEventReport(EventReport eventReport) {
      ...
      if (!eventReport.getDomainNames().isEmpty()) {
         for (String domain : eventReport.getDomainNames().toArray(new String[0])) {
            tagWithText(ELEMENT_DOMAIN, domain);
         }
      }

      if (!eventReport.getIps().isEmpty()) {
         for (String ip : eventReport.getIps().toArray(new String[0])) {
            tagWithText(ELEMENT_IP, ip);
         }
      }

      if (!eventReport.getMachines().isEmpty()) {
         for (Machine machine : eventReport.getMachines().values().toArray(new Machine[0])) {
   ...
   public void visitMachine(Machine machine) {
      ...
      if (!machine.getTypes().isEmpty()) {
         for (EventType type : machine.getTypes().values().toArray(new EventType[0])) {
            visitType(type);
         }
      }
   ...
   public void visitName(EventName name) {
      startTag(ENTITY_NAME, null, ATTR_ID, name.getId(), ATTR_TOTALCOUNT, name.getTotalCount(), ATTR_FAILCOUNT, name.getFailCount(), ATTR_FAILPERCENT, toString(name.getFailPercent(), "0.00"));
   ...
   public void visitType(EventType type) {
      startTag(ENTITY_TYPE, null, ATTR_ID, type.getId(), ATTR_TOTALCOUNT, type.getTotalCount(), ATTR_FAILCOUNT, type.getFailCount(), ATTR_FAILPERCENT, toString(type.getFailPercent(), "0.00"));
      ...
      if (!type.getNames().isEmpty()) {
         for (EventName name : type.getNames().values().toArray(new EventName[0])) {
            visitName(name);
         }
      }
